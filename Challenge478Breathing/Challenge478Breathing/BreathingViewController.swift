...
class BreathingViewController: UIViewController {
    ...
    func runBreatheTimer() {
        timer = Timer.scheduledTimer(timeInterval: 0.1, target: self, selector: #selector(breatheCountdown), userInfo: nil, repeats: true)
        ActLabel.text = "INHALE THROUGH YOUR NOSE NOW"
    }
    
    func runHoldTimer(){
        timer = Timer.scheduledTimer(timeInterval: 0.1, target: self, selector: #selector(holdCountdown), userInfo: nil, repeats: true)
        ActLabel.text = "HOLD YOUR BREATH NOW"
    }
    
    func runExhaleTimer(){
        timer = Timer.scheduledTimer(timeInterval: 0.1, target: self, selector: #selector(exhaleCountdown), userInfo: nil, repeats: true)
